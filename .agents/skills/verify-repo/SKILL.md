...
| `abyss-protocol-godot/**` | In that directory, run `python3 verify.py` for import and the smoke, weapons, campaign/persistence, Chinese story/UI, player services, safe-point continuation, animation, presentation, aiming, local co-op, expanded level/map geometry, balance and legacy/reward checkpoint suites. The wrapper also fails on script errors, timeouts and missing test summaries. Launch the native game for visual/input changes; `tests/coop.gd` covers two-device input and QHD rendering. Use isolated test saves. This native Godot project is not a Pages target and has no generated HTML artifact. |

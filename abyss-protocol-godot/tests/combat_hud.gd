...
func glaive_flight_status() -> void:
	...
	check(game.hud.skills[0].text == "回收\n" + game.CONTROLS.PAD_LABELS.slash, "Outbound glaive offers recall after recovery instead of another throw")
	check(game.hud.skill_icons[0].modulate == game.hud.partner_skill_icons[0].modulate, "Available recall and the other seat's attack both have ready icons")
